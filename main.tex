...
\end{split}
\end{equation*}
Quite evidently, the pmf depends only on $s$, not $t$, so:
$$
...
$$
The first equality arises from the fact that the complement of $(n,n+1,n+2,\dots)$ is $(n-1,n-2,\dots)$. This is equivalent to splitting $s$ into $n$ intervals, each having width equal to $T_1,T_2,\dots,T_n$. Thus:
$$
...
$$
P(E_{k+1}=e \mid X_k=x)=P\left(Y_{e,k}< \min_{\small\left[
\begin{matrix}
e'\in \Gamma(x) \cr
e'\neq e
\end{matrix}\right]
...
\dots&&
P\left(E_{k+1}=m  \mid  X_k=1\right)\cr\cr
P\left(E_{k+1}=1  \mid  X_k=2\right)&&
P\left(E_{k+1}=2  \mid  X_k=2\right)&&
\dots&&
P\left(E_{k+1}=m  \mid  X_k=2\right)\cr\cr
\vdots&&
\vdots&&
\ddots&&
\vdots\cr\cr
...
\dots&&
P\left(X_{k+1}=n  \mid  X_k=1\right)\cr\cr
P\left(X_{k+1}=1  \mid  X_k=2\right)&&
P\left(X_{k+1}=2  \mid  X_k=2\right)&&
\dots&&
P\left(X_{k+1}=n  \mid  X_k=2\right)\cr\cr
\vdots&&
\vdots&&
\ddots&&
\vdots\cr\cr
...
\underbrace{P\left(X_{k+1}=j  \mid  X_k=i\right)}_{(i,j)-th\text{ entry of }P_X}
...
$$
P\left(E_{k+1}=e\right)=\sum_{x\in\mathcal{X}}{P\left(E_{k+1}=e \mid X_k=x\right)P\left(X_k=x\right)}
$$
