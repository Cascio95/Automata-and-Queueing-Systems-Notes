...
\usepackage[hyperfootnotes=false]{hyperref} %Per rendere l'indice cliccabile. Da rimuovere quando stampo.
\hypersetup{
...
\begin{figure}[H]
	...
	\tikzset{edge/.style = {->,> = angle 60, thick}}
		...
		\node[rectangle,text height=1cm,text depth=1cm,inner ysep=0pt] at (4,0) (O) {$x_0, \, x_1, \, x_2, \, \dots, \, x_n$};
		\path[->]
		(I) edge  node[auto] {} (S)
		(S) edge  node[auto] {} (O)
		;
...
\begin{figure}[H]
	...
	\tikzset{edge/.style = {->,> = angle 60, thick}}
		...
		\node[rectangle,text height=1cm,text depth=1cm,inner ysep=0pt] at (4.5,0) (O) {
		$		
		\begin{matrix}
		...
		\end{matrix}
		$};
		\node[rectangle,text height=1cm,text depth=1cm,inner ysep=0pt] at (7.3,0) (d){}; %invisible node to center the picture
		\path[->]
		(I) edge  node[auto] {} (S)
		(S) edge  node[auto] {} (O)
		;
...
It’s a model through which Discrete Event Systems can be represented, when the elements of uncertainty must be taken into account, and it’s identified as a $6$-tuple $(\mathcal{E},\mathcal{X},\Gamma,\transp,\transp_0,F)$ where:
\begin{itemize}
...
\end{equation*}
\item $F$ is the stochastic clock structure and $F_e$ is the cumulative distribution function of the lifetimes of the generic event $e\in\mathcal{E}$:
\begin{equation*}
...
$(\mathcal{E},\mathcal{X},\Gamma,\transp,\transp_0,F)$ where $F = \{F_e : e \in \mathcal{E}\}$, with \textit{i.i.d.} events' lifetimes and:
$$
...
\end{matrix}
\right]
\end{equation*}\label{eq:stateprobs}

\noindent
Thus, it's possible to redefine $P(X_{k+1} = x' \mid X_k = x)$ as the probability that the $k+1$-th state is $j$ as:
\begin{equation*}
P(X_{k+1} = j) = 
\sum_{i \in \mathcal{X}} \underbrace{P(X_{k+1} = j \mid X_k = i)}_{ (i,j) \text{-th entry of } P_X} \underbrace{P(X_k = i)}_{i \text{-th entry of } \Pi_X(k)}
\end{equation*}
...
&&\vdots\\
\Pi_X(k+1)&=&\Pi_X(k) \cdot P_X&=&\Pi_X(0)P_X^{k+1}
\end{matrix}
$$
...
\right)=
\end{aligned}
\addtocounter{footnote}{-2} % -2 perchè sono 3 notes
...
\end{aligned}
\addtocounter{footnote}{-1} % -1 perchè sono 2 notes
...
\Pi(0)\cdot e^{Qt}
$$
The state probabilities can be divided, depending on the behaviour (with respect to time) of the system, in two categories: ‘Transient State’ and ‘Steady State’ probabilities:
\begin{itemize}
...
$$
\exists \, s : \transp_{i,j}(s)>0.
$$
Such concept can be also informally explained with the following definition: \textit{it must exist a directed path from state $i$ to state $j$}.
\item \textbf{Closure}: A subset $S\subseteq\mathcal{X}$ is \textit{closed} if 
$$
\transp_{i,j}=0,\hspace{10pt} \forall i\in S,\hspace{10pt} j\in\mathcal{X} \setminus S.
$$
...
\begin{matrix}
\begin{cases}
1 & \textrm{ then the system will return (for }t\rightarrow\infty\textrm{) to the state i}\\
...
\begin{matrix}
\begin{cases}
\infty & \textrm{then the integral doesn't converge}\\
&\textrm{and the state }i\textrm{ is said `null recurrent'.}\\
a<\infty & \textrm{then the integral converges and the}\\
& \textrm{state }i\textrm{ is said `positive recurrent'.}\\
\end{cases}
\end{matrix}
$$
...
$$
\newpage
\paragraph{Kendall's Notation} It describes queueing systems where there exists only one kind of customer, the scheduling policy is FIFO and a customer is rejected when the queue is full. 
