...
%\end{matrix}
%$$
% TODO Sostituire \begin matrix con questo in basso?
$$
...
\end{flushright}
\end{itemize}
% TODO c'è da aggiustare s con t; in alcuni punti manca s o t, o sono scambiati
...
\end{equation}
\noindent
% TODO forse è meglio essere sintetici
%Where $\Lambda(x)$ comes from $\min_{\dots}\left\lbrace Y_{e',k}\right\rbrace \sim Exp\left(1/\left(\Lambda(x) - \lambda_e\right)\right)$. $\Lambda(x)$ was inserted in order to ease the computation and it's defined as:
%$$
...
%e'\neq e
%\end{matrix}\right]
%}{\left\lbrace Y_{e',k}\right\rbrace }) \sim Exp\left(\frac{1}{\underset{{\left[
%\begin{matrix}
%e'\in \Gamma(x) \\
%e'\neq e
%\end{matrix}\right]
...
The state holding time $V(x)$ is a continuous random variable characterising the time spent by the system in the state $x$ (for a stochastic timed automaton with Poisson clock structure). Notice that, while the system is in state $x$, there might occur events which do not trigger a state transition: this implies that $V(x)$ keeps increasing until the system leaves state $x$.
% TODO Meglio detto cosi?
%, so $V(x)$ is a continuous random variable. Moreover, when computing a state holding time, it's fundamental to keep in mind the fact that, while the system is in state $x$, events can occur without causing a state transition and, therefore, without affecting the value of $V(x)$.
Let's compute the CDF of the state holding time and show that it is exponentially distributed with rate:
$$
\sum_{\mathlarger{e\in\Gamma(x)}}{\lambda_e
\left[
1-\transp\left(x \mid x,e\right)
\right]
}
$$
\emph{Proof:}
...
$$
For simplicity, consider $P(V(x) > t)$ only:
\begin{equation*}
\begin{split}
&P(V(x)> t) = P\left(\textit{no state transitions over the }\left(0,t\right] \textit{ interval} \mid X(0)=x\right)= \\
&=P\left(\underset{e\in\Gamma(x)}\bigcap\textit{no state transition triggered by event e over }\left( 0,t\right] \mid X(0)=x\right)=
\footnote[1]{Thanks to the independence of the events' lifetimes in Poisson clock structure} \\
&=\prod_{e\in\Gamma(x)}
P\left(\textit{no state transition triggered by event e over }\left( 0,t\right] \mid X(0)=x\right)= \\
&=\textit{it may occur more than once} = \\
&=\prod_{e\in\Gamma(x)}
P\left(
\begin{split}
...
\end{split}
\right)
\overset{\textit{union of}}{\underset{\textit{disjoint events}}{=}} \\
...
\end{equation*}
\noindent
In this last probability the occurrences of an \textit{i.i.d} set of exponentially distributed random variables are counted, which is equivalent to a Poisson counting process. Therefore, it's possible to use the equation \ref{eq:inxnextstateise} 
\begin{equation*}
\begin{split}
...
\end{equation*}
\noindent
Now the last sum can be rewritten as $e^x=\sum_{n=0}^{+\infty}x^n/n!$ (definition of the exponential):
\begin{equation*}
\begin{split}
...
\end{equation*}
\noindent
Switching to the complement of $P(V(x)) > t$ is straightforward, thus:
\begin{equation*}
